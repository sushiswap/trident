...
describe.only("Concentrated liquidity pool", function () {
  ...
  describe("pool #0", async () => {
    ...
    it("should add liquidity inside price range", async () => {
      ...
      expect(
        (await bento.balanceOf(dai.address, pool0.address)).toString()
      ).to.be.eq(
      ...
      expect(
        (await bento.balanceOf(weth.address, pool0.address)).toString()
      ).to.be.eq(
        dy.toString(),
  ...
  describe("pool #1", async () => {
    ...
    it("Should execute trade within current tick - one for zero", async () => {
      ...
      const tradePrice =
        parseInt(usdPaid.mul(100000).div(ethReceived)) / 100000;
    ...
    it("should execute trade within current tick - zero for one", async () => {
      ...
      const tradePrice =
        parseInt(usdReceived.mul(100000).div(ethPaid)) / 100000;
      const tradePriceSqrtX96 = getSqrtX96Price(tradePrice);
    ...
    it("Swap should work", async () => {
      ...
      const swapData = ethers.utils.defaultAbiCoder.encode(
        ["bool", "uint256", "address", "bool"],
        [true, getBigNumber(1), alice.address, false]
      );
      const oldBalance = await bento.balanceOf(usd.address, alice.address);
      await pool1.swap(swapData);
      const newBalance = await bento.balanceOf(usd.address, alice.address);
      expect(oldBalance.lt(newBalance)).to.be.true;
    });
  });
});
