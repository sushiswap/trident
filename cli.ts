...
task("router:add-liquidity", "Router add liquidity")
  ...
  .setAction(async function ({ tokenA, tokenB, pool }, { ethers, run, getChainId }, runSuper) {
    ...
    await (await bentoBox.connect(dev).deposit(liquidityInput[0].token, dev.address, dev.address, liquidityInput[0].amount, 0)).wait();
    await (await bentoBox.connect(dev).deposit(liquidityInput[1].token, dev.address, dev.address, liquidityInput[1].amount, 0)).wait();
