...
abstract contract ConcentratedLiquidityPool is IPool {
    ...
    function updateLinkedList(
        ...
        if (ticks[lower].liquidity > 0 || lower == TickMath.MIN_TICK) {
            ...
            ticks[lower].liquidity += uint112(amount);
        } else {
            ...
            if (lower <= currentNearestTick) {
                ticks[lower] = Tick(lowerOld, old.nextTick, uint112(amount), feeGrowthGlobal0X128, feeGrowthGlobal1X128);
            } else {
                ticks[lower] = Tick(lowerOld, old.nextTick, uint112(amount), 0, 0);
            }
        ...
        if (ticks[upper].liquidity > 0 || upper == TickMath.MAX_TICK) {
            // @dev we are adding liquidity to an existing tick.
            ticks[upper].liquidity += uint112(amount);
        } else {
            // @dev inserting a new tick.
            ...
            if (upper <= currentNearestTick) {
                ticks[upper] = Tick(upperOld, old.nextTick, uint112(amount), feeGrowthGlobal0X128, feeGrowthGlobal1X128);
            } else {
                ticks[upper] = Tick(upperOld, old.nextTick, uint112(amount), 0, 0);
            }
