...
contract FranchisedHybridPool is IPool, TridentFranchisedERC20 {
    ...
    constructor(bytes memory _deployData, IMasterDeployer _masterDeployer) {
        (address _token0, address _token1, uint256 _swapFee, uint256 a, IWhiteListManager _whiteListManager, address _operator, bool _level2) = abi.decode(
            _deployData,
            (address, address, uint256, uint256, IWhiteListManager, address, bool)
        );
        ...
        A = a;
        N_A = 2 * a;
        token0PrecisionMultiplier = uint256(10)**(decimals - TridentFranchisedERC20(_token0).decimals());
        token1PrecisionMultiplier = uint256(10)**(decimals - TridentFranchisedERC20(_token1).decimals());
        unlocked = 1;
    }
