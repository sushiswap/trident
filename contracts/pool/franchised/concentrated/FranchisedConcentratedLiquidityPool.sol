...
/// @notice Trident exchange franchised pool template implementing concentrated liquidity for swapping between an ERC-20 token pair.
/// @dev The reserves are stored as bento shares.
///      The curve is applied to shares as well. This pool does not care about the underlying amounts.
contract FranchisedConcentratedLiquidityPool is IPool {
    ...
    bytes32 public constant override poolIdentifier = "Trident:FranchisedCL";
    ...
    constructor(bytes memory _deployData, IMasterDeployer _masterDeployer, IWhiteListManager _whiteListManager) {
        (
            address _token0, 
            address _token1, 
            uint24 _swapFee, 
    ...
    function mint(bytes calldata data) public override lock returns (uint256 _liquidity) {
        ...
        whiteListManager.whitelistedAccounts(operator, mintParams.positionRecipient);
    ...
    /// @notice Burn function that cannot conform to the IPool interface due to having three return values.
    ...
    {
        whiteListManager.whitelistedAccounts(operator, recipient);
        uint256 amount0;
        uint256 amount1;
    ...
    function swap(bytes memory data) public override lock returns (uint256 amountOut) {
        ...
        if (level2) whiteListManager.whitelistedAccounts(operator, recipient);
    ...
    /// @dev Updates `swapFee` for pool.
    function updateSwapFee(uint24 _swapFee) public {
        if (msg.sender != operator) revert Unauthorized();
        swapFee = _swapFee;
    }
    
    /// @dev Updates `level2` status for pool.
    function updateLevel2(bool _level2) public {
        if (msg.sender != operator) revert Unauthorized();
        level2 = _level2;
    }
