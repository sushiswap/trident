...
contract FranchisedIndexPool is IPool, TridentFranchisedERC20 {
    ...
    constructor(bytes memory _deployData, IMasterDeployer _masterDeployer) {
        (address[] memory _tokens, uint136[] memory _weights, uint256 _swapFee, IWhiteListManager _whiteListManager, address _operator, bool _level2) = abi.decode(
            _deployData,
            (address[], uint136[], uint256, IWhiteListManager, address, bool)
        );
