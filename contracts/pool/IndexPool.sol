...
contract IndexPool is IPool, TridentERC20 {
    ...
    constructor(bytes memory _deployData, address _masterDeployer) {
        (address[] memory _tokens, uint256[] memory _weights, uint256 _swapFee) = abi.decode(_deployData, (address[], uint256[], uint256));
    ...
    function burnSingle(bytes calldata data) public override lock returns (uint256 amount) {
        ...
        amount = _computeSingleOutGivenPoolIn(outRecord.balance, outRecord.weight, totalSupply, totalWeight, toBurn, swapFee);
    ...
    function getAmountOut(bytes calldata data) public view override returns (uint256 amountOut) {
        (uint256 tokenInBalance, uint256 tokenInWeight, uint256 tokenOutBalance, uint256 tokenOutWeight, uint256 tokenInAmount) = abi.decode(
            data,
